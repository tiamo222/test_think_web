...
class wysiwygPro {
	...
	*/
	...
	function wysiwygPro($save_name='') {
		...
		if (!defined('WP_CONFIG')) {
			die('<p><b>WYSIWYG - Setup Error:</b> You did not include \'config.php\' before declaring this class!</p>');
		}	
	...
	function addbutton($title, $location, $function, $url, $width=NULL, $height=NULL, $cid='forecolor') {
		if ($this->has_expired) {
			...
			if (substr($location, 0, 6) == 'after:') {
				...
				die("<p><b>Wysiwyg Paramater Error:</b> You supplied an incorrect argument to addbuttons</p>");
			}
		}
	}
	...
	*/
	...
	function set_classmenu($classes) {
		if ($this->has_expired) {
			if (!is_array($classes)) {
				die('<p><b>WYSIWYG Paramater Error:</b> Your class list is not an array!</p>');
			} else {
				$class_menu = '';
