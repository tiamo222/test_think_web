...
*/
	 ...
	 *
	 * @param d	a date in Unix date time format. dd/mm/yyyy
	 *
	 * @return  date string in database date format
	 */
	function VarcharDate($d){
		$result = "";
		if (ereg("([0-9]{2})/([0-9]{2})/([0-9]{4})", $d, $regs)) {
			$result =  ($regs[3]-543).$regs[2].$regs[1];
		}
		return $result;
	}
	/**
	...
	function TDBDate($d)
	{
		...
		if(is_string($d)) $arr = split("/",$d);
		if(is_array($arr)) $d=($arr[2]-543)."-".$arr[1]."-".$arr[0];
		
		return $this->DBDate($d);
	}
	function TDBTimeStamp($d,$t="")
	{
		if(is_null($d)||$d=="") return "NULL";
		if(is_string($d)) $arr = split("/",$d);
		if(is_array($arr)) $d=($arr[2]-543)."-".$arr[1]."-".$arr[0];

		return $this->DBTimeStamp($d.$t);
	}
	function BindDate($d)
	{
		$d = $this->DBDate($d);
	...
	function TUserDate($v)
	{
		if(is_null($v)|| $v=="") return "";
		$d = $this->UserDate($v,"d-m-Y");
		if(is_string($d)) $arr = split("-",$d);
		if(is_array($arr)) $d=$arr[0]."/".$arr[1]."/".(is_numeric($arr[2])?$arr[2]+543 :$arr[2]);
		return $d;
	}
